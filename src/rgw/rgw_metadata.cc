...
int RGWMetadataLog::store_entries_in_shard(list <cls_log_entry> &entries, int shard_id,
...
int RGWMetadataManager::post_modify(RGWMetadataHandler *handler, const string &section, const string &key,
                                    RGWMetadataLogData &log_data,
...
int RGWMetadataManager::remove_from_heap(RGWMetadataHandler *handler, const string &key,
                                         RGWObjVersionTracker *objv_tracker) {
    if (!objv_tracker) {
        return -EINVAL;
    }
...
int RGWMetadataManager::put_entry(RGWMetadataHandler *handler, const string &key, bufferlist &bl, bool exclusive,
    ...
    if (ret < 0) {
        ldout(store->ctx(), 0) << "ERROR: " << __func__ << ": store_in_heap() key=" << key << " returned ret=" << ret
                               << dendl;
        goto done;
    }
    ...
    if (ret < 0) {
        ...
        if (r < 0) {
            ldout(store->ctx(), 0) << "ERROR: " << __func__ << ": remove_from_heap() key=" << key << " returned ret="
                                   << r << dendl;
        }
    }
    done:
