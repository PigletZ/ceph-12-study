...
public:
    RGWCoroutine(CephContext *_cct) : status(_cct), _yield_ret(false), cct(_cct), stack(NULL), retcode(0),
    ...
    bool
    collect_next(int *ret, RGWCoroutinesStack **collected_stack = NULL); /* returns true if found a stack to collect */

    int wait(const utime_t &interval);

    bool drain_children(int num_cr_left,
                        RGWCoroutinesStack *skip_stack = NULL); /* returns true if needed to be called again */
    void wakeup();
...
protected:
    ...
    bool
    collect(RGWCoroutine *op, int *ret, RGWCoroutinesStack *skip_stack); /* returns true if needs to be called again */
...
class RGWCoroutinesManager {
    ...
    void handle_unblocked_stack(set<RGWCoroutinesStack *> &context_stacks, list<RGWCoroutinesStack *> &scheduled_stacks,
                                RGWCoroutinesStack *stack, int *waiting_count);
