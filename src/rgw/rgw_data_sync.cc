...
bool RGWReadDataSyncStatusMarkersCR::spawn_next() {
    ...
    spawn(new CR(env->async_rados, env->store,
                 rgw_raw_obj(env->store->get_zone_params().log_pool,
...
public:
    RGWReadRemoteDataLogInfoCR(RGWDataSyncEnv *_sync_env,
                               int _num_shards,
                               map<int, RGWDataChangesLogInfo> *_datalog_info) : RGWShardCollectCR(_sync_env->cct,
...
public:
    RGWListRemoteDataLogCR(RGWDataSyncEnv *_sync_env,
                           ...
                           map<int, rgw_datalog_shard_data> *_result) : RGWShardCollectCR(_sync_env->cct,
                                                                                          READ_DATALOG_MAX_CONCURRENT),
                                                                        sync_env(_sync_env),
...
bool RGWListRemoteDataLogCR::spawn_next() {
    ...
    spawn(new RGWListRemoteDataLogShardCR(sync_env, iter->first, iter->second, max_entries_per_shard,
                                          &(*result)[iter->first]), false);
    ++iter;
    return true;
}
    ...
    int operate() override {
        ...
        reenter(this)
        {
            ...
            yield{
                    ...
                    if (!conn) {
                        ldout(cct, 0) << "ERROR: connection to zone " << sync_env->source_zone << " does not exist!"
                                      << dendl;
...
int RGWRemoteDataLog::read_source_log_shards_next(map<int, string> shard_markers,
                                                  map<int, rgw_datalog_shard_data> *result) {
...
public:
    RGWListBucketIndexesCR(RGWDataSyncEnv *_sync_env,
                           rgw_data_sync_status *_sync_status) : RGWCoroutine(_sync_env->cct), sync_env(_sync_env),
                                                                 ...
                                                                 req_ret(0), ret(0), entries_index(NULL), i(0),
                                                                 failed(false) {
                ...
                if (num_shards > 0) {
                    for (i = 0; i < num_shards; i++) {
                        ...
                        entries_index->append(s,
                                              store->data_log->get_log_shard_id(meta_info.data.get_bucket_info().bucket,
                                                                                i));
                    }
                } else {
                    yield
                    entries_index->append(key,
                                          store->data_log->get_log_shard_id(meta_info.data.get_bucket_info().bucket,
                                                                            -1));
                }
            }
            ...
            if (!failed) {
                for (map<uint32_t, rgw_data_sync_marker>::iterator iter = sync_status->sync_markers.begin();
                    ...
                    spawn(new RGWSimpleRadosWriteCR<rgw_data_sync_marker>(sync_env->async_rados, store,
                                                                          rgw_raw_obj(store->get_zone_params().log_pool,
...
public:
    RGWDataSyncShardMarkerTrack(RGWDataSyncEnv *_sync_env,
                                ...
                                const rgw_data_sync_marker &_marker) : RGWSyncShardMarkerTrack(
            DATA_SYNC_UPDATE_MARKER_WINDOW),
                                                                       sync_env(_sync_env),
    ...
    RGWCoroutine *store_marker(const string &new_marker, uint64_t index_pos, const real_time &timestamp) override {
        ...
        ldout(sync_env->cct, 20) << __func__ << "(): updating marker marker_oid=" << marker_oid << " marker="
                                 << new_marker << dendl;
        RGWRados *store = sync_env->store;
...
public:
    RGWDataSyncSingleEntryCR(RGWDataSyncEnv *_sync_env,
        ...
        set_description() << "data sync single entry (source_zone=" << sync_env->source_zone << ") key=" << _raw_key
                          << " entry=" << entry_marker;
    }
        ...
        reenter(this)
        {
            ...
            if (sync_status < 0) {
                ...
                if (sync_status != -EBUSY && sync_status != -EAGAIN) {
                    ...
                    if (retcode < 0) {
                        ldout(sync_env->store->ctx(), 0) << "ERROR: failed to log sync failure: retcode=" << retcode
                                                         << dendl;
                    }
                }
                if (error_repo && !error_repo->append(raw_key)) {
                    ldout(sync_env->store->ctx(), 0) << "ERROR: failed to log sync failure in error repo: retcode="
                                                     << retcode << dendl;
                }
...
public:
    RGWDataSyncShardCR(RGWDataSyncEnv *_sync_env,
                       rgw_pool &_pool,
                       uint32_t _shard_id, const rgw_data_sync_marker &_marker, bool *_reset_backoff) : RGWCoroutine(
            _sync_env->cct),
                                                                                                        sync_env(
                                                                                                                _sync_env),
                                                                                                        pool(_pool),
...
#define OMAP_GET_MAX_ENTRIES 100
        ...
        reenter(&full_cr)
        {
            ...
            do {
                ...
                yield call(
                        new RGWRadosGetOmapKeysCR(sync_env->store, rgw_raw_obj(pool, oid), sync_marker.marker, &entries,
                                                  max_entries));
                if (retcode < 0) {
                    ldout(sync_env->cct, 0) << "ERROR: " << __func__ << "(): RGWRadosGetOmapKeysCR() returned ret="
                                            << retcode << dendl;
                    lease_cr->go_down();
                    drain_all();
                    return set_cr_error(retcode);
                ...
                for (; iter != entries.end(); ++iter) {
                    ...
                    if (!marker_tracker->start(*iter, total_entries, real_time())) {
                        ldout(sync_env->cct, 0) << "ERROR: cannot start syncing " << *iter << ". Duplicate entry?"
                                                << dendl;
                    } else {
                        // fetch remote and write locally
                        yield spawn(
                                new RGWDataSyncSingleEntryCR(sync_env, *iter, *iter, marker_tracker, error_repo, false),
                                false);
                    }
        ...
        reenter(&incremental_cr)
        {
            ...
            do {
                ...
                for (modified_iter = current_modified.begin();
                    ...
                    yield{
                            ldout(sync_env->cct, 20) << __func__ << "(): async update notification: " << *modified_iter
                                                     << dendl;
                ...
                if (error_retry_time <= ceph::coarse_real_clock::now()) {
                    ...
                    yield call(new RGWRadosGetOmapKeysCR(sync_env->store, rgw_raw_obj(pool, error_oid),
                                                         ...
                                                         max_error_entries));
                    ldout(sync_env->cct, 20) << __func__ << "(): read error repo, got " << error_entries.size()
                                             << " entries" << dendl;
                    ...
                    for (; iter != error_entries.end(); ++iter) {
                        ...
                        spawn(new RGWDataSyncSingleEntryCR(sync_env, error_marker, error_marker,
                                                           nullptr /* no marker tracker */, error_repo, true), false);
                    }
...
#define INCREMENTAL_MAX_ENTRIES 100
                ldout(sync_env->cct, 20) << __func__ << ":" << __LINE__ << ": shard_id=" << shard_id << " sync_marker="
                                         << sync_marker.marker << dendl;
                spawned_keys.clear();
                ...
                for (log_iter = log_entries.begin(); log_iter != log_entries.end(); ++log_iter) {
                    ldout(sync_env->cct, 20) << __func__ << ":" << __LINE__ << ": shard_id=" << shard_id
                                             << " log_entry: " << log_iter->log_id << ":" << log_iter->log_timestamp
                                             << ":" << log_iter->entry.key << dendl;
                    if (!marker_tracker->index_key_to_marker(log_iter->entry.key, log_iter->log_id)) {
                        ldout(sync_env->cct, 20) << __func__ << ": skipping sync of entry: " << log_iter->log_id << ":"
                                                 << log_iter->entry.key << " sync already in progress for bucket shard"
                                                 << dendl;
                    ...
                    if (!marker_tracker->start(log_iter->log_id, 0, log_iter->log_timestamp)) {
                        ldout(sync_env->cct, 0) << "ERROR: cannot start syncing " << log_iter->log_id
             ...
             */
                        if (spawned_keys.find(log_iter->entry.key) == spawned_keys.end()) {
                            ...
                            spawn(new RGWDataSyncSingleEntryCR(sync_env, log_iter->entry.key, log_iter->log_id,
                                                               marker_tracker, error_repo, false), false);
                            if (retcode < 0) {
                                stop_spawned_services();
                                drain_all();
                                return set_cr_error(retcode);
...
public:
    RGWDataSyncCR(RGWDataSyncEnv *_sync_env, uint32_t _num_shards, bool *_reset_backoff) : RGWCoroutine(_sync_env->cct),
                                                                                           ...
                                                                                           marker_tracker(NULL),
                                                                                           shard_crs_lock(
                                                                                                   "RGWDataSyncCR::shard_crs_lock"),
                                                                                           reset_backoff(
                                                                                                   _reset_backoff),
                                                                                           logger(sync_env, "Data",
                                                                                                  "all") {
        ...
        reenter(this)
        {
            ...
            yield{
                    if ((rgw_data_sync_info::SyncState)sync_status.sync_info.state == rgw_data_sync_info::StateSync) {
                        for (map<uint32_t, rgw_data_sync_marker>::iterator iter = sync_status.sync_markers.begin();
                             iter != sync_status.sync_markers.end(); ++iter) {
                            RGWDataSyncShardControlCR *cr = new RGWDataSyncShardControlCR(sync_env,
    ...
    RGWCoroutine *set_sync_info_cr() {
        ...
        return new RGWSimpleRadosWriteCR<rgw_data_sync_info>(sync_env->async_rados, store,
                                                             rgw_raw_obj(store->get_zone_params().log_pool,
                                                                         RGWDataSyncStatusManager::sync_status_oid(
                                                                                 sync_env->source_zone)),
...
public:
    ...
    RGWCoroutine *sync_object(RGWDataSyncEnv *sync_env, RGWBucketInfo &bucket_info, rgw_obj_key &key,
                              boost::optional <uint64_t> versioned_epoch, rgw_zone_set *zones_trace) override;

    RGWCoroutine *
    remove_object(RGWDataSyncEnv *sync_env, RGWBucketInfo &bucket_info, rgw_obj_key &key, real_time &mtime,
                  bool versioned, uint64_t versioned_epoch, rgw_zone_set *zones_trace) override;

    RGWCoroutine *
    create_delete_marker(RGWDataSyncEnv *sync_env, RGWBucketInfo &bucket_info, rgw_obj_key &key, real_time &mtime,
...
int RGWDefaultSyncModule::create_instance(CephContext *cct, map <string, string, ltstr_nocase> &config,
...
RGWCoroutine *
RGWDefaultDataSyncModule::sync_object(RGWDataSyncEnv *sync_env, RGWBucketInfo &bucket_info, rgw_obj_key &key,
                                      boost::optional <uint64_t> versioned_epoch, rgw_zone_set *zones_trace) {
...
RGWCoroutine *
RGWDefaultDataSyncModule::remove_object(RGWDataSyncEnv *sync_env, RGWBucketInfo &bucket_info, rgw_obj_key &key,
...
RGWCoroutine *
RGWDefaultDataSyncModule::create_delete_marker(RGWDataSyncEnv *sync_env, RGWBucketInfo &bucket_info, rgw_obj_key &key,
                                               real_time &mtime,
                                               rgw_bucket_entry_owner &owner, bool versioned, uint64_t versioned_epoch,
                                               rgw_zone_set *zones_trace) {
    return new RGWRemoveObjCR(sync_env->async_rados, sync_env->store, sync_env->source_zone,
...
int RGWReadBucketSyncStatusCoroutine::operate() {
    reenter(this)
    {
        ...
        if (retcode < 0) {
            ldout(sync_env->cct, 0) << "ERROR: failed to call fetch bucket shard info oid=" << oid << " ret=" << retcode
                                    << dendl;
            return set_cr_error(retcode);
        }
...
int RGWRemoteDataLog::read_shard_status(int shard_id, set <string> &pending_buckets, set <string> &recovering_buckets,
                                        rgw_data_sync_marker *sync_marker, const int max_entries) {
    // cannot run concurrently with run_sync(), so run in a separate manager
    RGWCoroutinesManager crs(store->ctx(), store->get_cr_registry());
    RGWHTTPManager http_manager(store->ctx(), crs.get_completion_mgr());
    int ret = http_manager.set_threaded();
    ...
    recovering_stack->call(
            new RGWReadRecoveringBucketShardsCoroutine(&sync_env_local, shard_id, recovering_buckets, max_entries));
    stacks.push_back(recovering_stack);
    RGWCoroutinesStack *pending_stack = new RGWCoroutinesStack(store->ctx(), &crs);
    pending_stack->call(new RGWReadPendingBucketShardsCoroutine(&sync_env_local, shard_id, pending_buckets, sync_marker,
                                                                max_entries));
...
public:
    ...
    int operate() override {
        reenter(this)
        {
            yield{
                    rgw_http_param_pair pairs[] = {
                        { "rgwx-bucket-instance", instance_key.c_str() },
                        { "versions", NULL },
                        { "format", "json" },
                        { "objs-container", "true" },
...
public:
    RGWBucketFullSyncShardMarkerTrack(RGWDataSyncEnv *_sync_env,
                                      const string &_marker_oid,
                                      const rgw_bucket_shard_full_sync_marker &_marker) : RGWSyncShardMarkerTrack(
            BUCKET_SYNC_UPDATE_MARKER_WINDOW),
                                                                                          sync_env(_sync_env),
                                                                                          marker_oid(_marker_oid),
                                                                                          sync_marker(_marker) {}
...
public:
    RGWBucketIncSyncShardMarkerTrack(RGWDataSyncEnv *_sync_env,
                                     const string &_marker_oid,
                                     const rgw_bucket_shard_inc_sync_marker &_marker) : RGWSyncShardMarkerTrack(
            BUCKET_SYNC_UPDATE_MARKER_WINDOW),
                                                                                        sync_env(_sync_env),
                                                                                        marker_oid(_marker_oid),
                                                                                        sync_marker(_marker) {}
...
public:
    RGWBucketSyncSingleEntryCR(RGWDataSyncEnv *_sync_env,
                               ...
                               rgw_zone_set &_zones_trace) : RGWCoroutine(_sync_env->cct),
                                                             sync_env(_sync_env),
        ...
        set_description() << "bucket sync single entry (source_zone=" << sync_env->source_zone << ") b=" << ss.str()
                          << " log_entry=" << entry_marker << " op=" << (int) op << " op_state=" << (int) op_state;
        ldout(sync_env->cct, 20) << "bucket sync single entry (source_zone=" << sync_env->source_zone << ") b="
                                 << ss.str() << " log_entry=" << entry_marker << " op=" << (int) op << " op_state="
                                 << (int) op_state << dendl;
        set_status("init");
    ...
    int operate() override {
        reenter(this)
        {
            ...
            do {
                yield{
                        ...
                        if (key.name.empty()) {
                            ...
                            ldout(sync_env->cct, 0) << "ERROR: " << __func__
                                                    << "(): entry with empty obj name, skipping" << dendl;
                            goto done;
                        }
                        if (error_injection &&
                        rand() % 10000 < cct->_conf->rgw_sync_data_inject_err_probability * 10000.0) {
                            ldout(sync_env->cct, 0) << __func__ << ": injecting data sync error on key=" << key.name
                                                    << dendl;
                            retcode = -EIO;
                        } else if (op == CLS_RGW_OP_ADD ||
                        op == CLS_RGW_OP_LINK_OLH) {
                            set_status("syncing obj");
                            ldout(sync_env->cct, 5) << "bucket sync: sync obj: " << sync_env->source_zone << "/"
                            ...
                            logger.log("remove");
                            call(data_sync_module->remove_object(sync_env, *bucket_info, key, timestamp, versioned,
                            ...
                            ldout(sync_env->cct, 10) << "creating delete marker: obj: " << sync_env->source_zone << "/"
                                                     << bucket_info->bucket << "/" << key << "["
                                                     << versioned_epoch.value_or(0) << "]" << dendl;
                            call(data_sync_module->create_delete_marker(sync_env, *bucket_info, key, timestamp, owner,
                                                                        versioned, versioned_epoch.value_or(0),
                                                                        &zones_trace));
                        }
                }
            ...
            if (!error_ss.str().empty()) {
                yield call(sync_env->error_logger->log_error_cr(sync_env->conn->get_remote_id(), "data", error_ss.str(),
                                                                -retcode, string("failed to sync object") +
                                                                          cpp_strerror(-sync_status)));
            }
            done:
            if (sync_status == 0) {
        ...
        do {
            ...
            for (; entries_iter != list_result.entries.end(); ++entries_iter) {
                ...
                if (!marker_tracker.start(entry->key, total_entries, real_time())) {
                    ldout(sync_env->cct, 0) << "ERROR: cannot start syncing " << entry->key << ". Duplicate entry?"
                                            << dendl;
                } else {
...
int RGWBucketShardIncrementalSyncCR::operate() {
    ...
    reenter(this)
    {
        do {
            ...
            ldout(sync_env->cct, 20) << __func__ << "(): listing bilog for incremental sync"
                                     << sync_info.inc_marker.position << dendl;
            ...
            for (; entries_iter != entries_end; ++entries_iter) {
                ...
                if (entry->op == RGWModifyOp::CLS_RGW_OP_SYNCSTOP || entry->op == RGWModifyOp::CLS_RGW_OP_RESYNC) {
                    ldout(sync_env->cct, 20) << "detected syncstop or resync on " << entries_iter->timestamp
                ...
                if (!key.set(rgw_obj_index_key{entry->object, entry->instance})) {
                    ...
                    ldout(sync_env->cct, 20) << "parse_raw_oid() on " << entry->object
                                             << " returned false, skipping entry" << dendl;
                    marker_tracker.try_update_high_marker(cur_id, 0, entry->timestamp);
                    continue;
                }

                ldout(sync_env->cct, 20) << "parsed entry: id=" << cur_id << " iter->object=" << entry->object
                                         << " iter->instance=" << entry->instance << " name=" << key.name
                                         << " instance=" << key.instance << " ns=" << key.ns << dendl;
                ...
                while (!marker_tracker.can_do_op(key, has_olh_epoch(entry->op))) {
                    ...
                    ldout(sync_env->cct, 5) << *this << ": [inc sync] can't do op on key=" << key
                    ...
                    while (again) {
                        again = collect(&ret, nullptr);
                        if (ret < 0) {
                            ldout(sync_env->cct, 0) << "ERROR: a child operation returned error (ret=" << ret << ")"
                                                    << dendl;
                            sync_status = ret;
                            /* we have reported this error */
                        }
                ...
                if (!marker_tracker.index_key_to_marker(key, cur_id, has_olh_epoch(entry->op))) {
                    ...
                    ldout(sync_env->cct, 20) << __func__ << ": skipping sync of entry: " << cur_id << ":" << key
                ...
                if (!marker_tracker.start(cur_id, 0, entry->timestamp)) {
                    ldout(sync_env->cct, 0) << "ERROR: cannot start syncing " << cur_id << ". Duplicate entry?"
                                            << dendl;
                } else {
                    boost::optional <uint64_t> versioned_epoch;
...
int RGWRunBucketSyncCoroutine::operate() {
    reenter(this)
    {
        ...
        if (retcode == -ENOENT) {
            ...
            if (retcode < 0) {
                ldout(sync_env->cct, 0) << "ERROR: failed to fetch bucket instance info for " << bucket_str{bs.bucket}
                                        << dendl;
                lease_cr->go_down();
                drain_all();
                return set_cr_error(retcode);
        ...
        if (retcode < 0) {
            ldout(sync_env->cct, 0) << "ERROR: failed to retrieve bucket info for bucket=" << bucket_str{bs.bucket}
                                    << dendl;
            lease_cr->go_down();
            drain_all();
            return set_cr_error(retcode);
...
int RGWBucketSyncStatusManager::init() {
    ...
    ret = cr_mgr.run(
            new RGWReadRESTResourceCR<bucket_instance_meta_info>(store->ctx(), conn, &http_manager, path, pairs,
                                                                 &result));
    if (ret < 0) {
        ldout(store->ctx(), 0) << "ERROR: failed to fetch bucket metadata info from zone=" << source_zone << " path="
                               << path << " key=" << key << " ret=" << ret << dendl;
        return ret;
    }
