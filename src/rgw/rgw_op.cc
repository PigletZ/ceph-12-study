...
static int
modify_obj_attr(RGWRados *store, struct req_state *s, rgw_obj &obj, const char *attr_name, bufferlist &attr_val) {
    map <string, bufferlist> attrs;
    RGWRados::Object op_target(store, s->bucket_info, *static_cast<RGWObjectCtx *>(s->obj_ctx), obj);
    RGWRados::Object::Read read_op(&op_target);
...
int rgw_build_bucket_policies(RGWRados *store, struct req_state *s) {
    ...
    if (!s->bucket_name.empty()) {
        ...
        if (s->bucket_instance_id.empty()) {
            ret = store->get_bucket_info(obj_ctx, s->bucket_tenant, s->bucket_name, s->bucket_info, NULL,
                                         &s->bucket_attrs);
        } else {
            ret = store->get_bucket_instance_info(obj_ctx, s->bucket_instance_id, s->bucket_info, NULL,
                                                  &s->bucket_attrs);
        }
        if (ret < 0) {
            if (ret != -ENOENT) {
        ...
        if (s->bucket_exists && !store->get_zonegroup().equals(s->bucket_info.zonegroup)) {
            ldout(s->cct, 0) << "NOTICE: request for data in a different zonegroup (" << s->bucket_info.zonegroup
...
int RGWOp::verify_op_mask() {
    ...
    if (!s->system_request && (required_mask & RGW_OP_TYPE_MODIFY) && !store->zone_is_writeable()) {
        ldout(s->cct, 5) << "NOTICE: modify request to a read-only zone by a non-system user, permission denied"
                         << dendl;
        return -EPERM;
    }
...
flags) {
dout(10) << "Method " << req_meth << " is supported" <<
dendl;
} else {
dout(5) << "Method " << req_meth << " is not supported" <<
dendl;
return false;
}
...
for(
list<string>::iterator it = hl.begin();
it != hl.
...
dout(5) << "Header " << (*it) << " is not registered in this rule" <<
dendl;
} else {
if (hdrs.
...
int RGWGetObj::read_user_manifest_part(rgw_bucket &bucket,
    ...
    if (need_decompress) {
        ...
        if (obj_size != ent.meta.size) {
            // hmm.. something wrong, object not as expected, abort!
            ldout(s->cct, 0) << "ERROR: expected obj_size=" << obj_size << ", actual read size=" << ent.meta.size
                             << dendl;
            return -EIO;
        }
    }
...
return op->
read_user_manifest_part(bucket, ent, bucket_acl, bucket_policy, start_ofs, end_ofs
);
}
...
void RGWSetBucketWebsite::execute() {
    ...
    if (op_ret < 0) {
        ldout(s->cct, 0) << "NOTICE: put_bucket_info on bucket=" << s->bucket.name << " returned err=" << op_ret
                         << dendl;
        return;
    }
}
...
RGWPutObjProcessor *RGWPutObj::select_processor(RGWObjectCtx &obj_ctx, bool *is_multipart) {
    ...
    if (!multipart) {
        processor = new RGWPutObjProcessor_Atomic(obj_ctx, s->bucket_info, s->bucket, s->object.name, part_size,
                                                  s->req_id, s->bucket_info.versioning_enabled());
...
void RGWCompleteMultipart::execute() {
    ...
    do {
        ...
        for (obj_iter = obj_parts.begin();
             iter != parts->parts.end() && obj_iter != obj_parts.end(); ++iter, ++obj_iter, ++handled_parts) {
